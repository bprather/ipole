...
REAL kappa_I(struct parameters * params)
{
  ...
  REAL nu_w = pow(params->kappa_width * params->kappa, 2.) *nu_c
                *sin(params->observer_angle);

  REAL X_k = params->nu/nu_w;
  ...
  REAL Nlow = 4. * params->pi * tgamma(params->kappa-4./3.) 
                / (pow(3., 7./3.) * tgamma(params->kappa-2.));

  REAL Nhigh = (1./4.) * pow(3., (params->kappa-1.)/2.) 
                 * (params->kappa-2.) * (params->kappa-1.)
		 * tgamma(params->kappa/4.-1./3.) 
                 * tgamma(params->kappa/4.+4./3.);

  REAL x = 3. * pow(params->kappa, -3./2.);

  REAL ans = prefactor * Nlow * pow(X_k, 1./3.) 
...
REAL kappa_Q(struct parameters * params)
{
  ...
  REAL Nlow = -(1./2.) * 4. * params->pi * tgamma(params->kappa-4./3.)
                 /(pow(3., 7./3.) * tgamma(params->kappa-2.));

  REAL Nhigh = -(pow(4./5., 2)+params->kappa/50.) * (1./4.) 
                 * pow(3., (params->kappa-1.)/2.) * (params->kappa-2.) 
                 * (params->kappa-1.) * tgamma(params->kappa/4.-1./3.)
                 * tgamma(params->kappa/4.+4./3.);
 
  REAL x = (37./10.)*pow(params->kappa, -8./5.);
...
REAL kappa_V(struct parameters * params)
{
  ...
  REAL nu_w = pow(params->kappa_width * params->kappa, 2.) 
                * nu_c * sin(params->observer_angle);
...
REAL kappa_I_abs(struct parameters * params)
{
  ...
  REAL prefactor = params->electron_density * params->electron_charge 
                     / (params->magnetic_field * sin(params->observer_angle));

  REAL a = params->kappa - 1./3.;
 
  REAL b = params->kappa + 1.;

  REAL c = params->kappa + 2./3.;

  REAL z = -params->kappa*params->kappa_width;

  /*GSL 2F1 only works for |z| < 1; had to apply a hypergeometric function
  identity because in our case z = -kappa*w, so |z| > 1 */
  REAL hyp2f1 = pow(1.-z, -a) * tgamma(c) * tgamma(b-a) 
                 / (tgamma(b)*tgamma(c-a)) 
  ...
  REAL Nlow = pow(3., 1./6.) * (10./41.) * pow(2. * params->pi, 2.)
  ...
  REAL Nhigh = 2. * pow(params->pi, 5./2.)/3. * (params->kappa-2.) 
  ...
  REAL x = pow(-7./4. + 8. * params->kappa/5., -43./50.);

  REAL ans = prefactor * Nlow * pow(X_k, -5./3.)
...
REAL kappa_Q_abs(struct parameters * params)
{
  ...
  REAL hyp2f1 = pow(1.-z, -a) * tgamma(c) * tgamma(b-a) 
                 ...
                 * gsl_sf_hyperg_2F1(b, c - a, b - a + 1., 1./(1. - z));
  REAL Nlow = -(25./48.) * pow(3., 1./6.) * (10./41.) 
                * pow(2. * params->pi, 2.)
                / pow(params->kappa_width*params->kappa, 16./3.-params->kappa)
                * (params->kappa-2.) * (params->kappa-1.)
                * params->kappa / (3. * params->kappa-1.) 
                * tgamma(5./3.) * hyp2f1;
  REAL Nhigh = -(pow(21., 2.) * pow(params->kappa, -144./25.) + 11./20.) 
  ...
  REAL x = (7./5.) * pow(params->kappa, -23./20.);
...
REAL kappa_V_abs(struct parameters * params)
{
  ...
  REAL Nlow = -(77./(100. * params->kappa_width)) 
  ...
  REAL Nhigh = -(143./10. * pow(params->kappa_width, -116./125.))
  ...
  REAL x = (61./50.)*pow(params->kappa, -142./125.)+7./1000.;
