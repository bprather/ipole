...
int integrate_emission(struct of_traj *traj, int nsteps,
                    ...
                    double complex N_coord[NDIM][NDIM], Params *params, int verbose)
{
  ...
  for (int nstep=nsteps; nstep > 0; --nstep) {
    ...
    if (radiating_region(tf.X)) {
      ...
      // TODO use this machinery for counterjet, too
      int ZERO_EMISSION = 0;
    ...
    complex double Nf[NDIM][NDIM], double dlam)
{
  ...
  if ( isnan(creal(Nf[0][0])) ) {
    fprintf(stderr, "\nNaN in N00 (parallel transport)!\n");
    fprintf(stderr, "dlam: %g dl: %g\n", dlam, dl);
    print_vector("Xi", Xi);
    print_vector("Xm", Xm);
    print_vector("Xf", Xf);
    print_vector("Ki", Ki);
    print_vector("Km", Km);
    print_vector("Kf", Km);
    print_matrix_c("Ni", Ni);
    print_matrix_c("Nm", Nm);
    print_matrix_c("Nf", Nf);
    exit(-1);
  }
...
int evolve_N(double Xi[NDIM], double Kconi[NDIM],
    ...
    int ZERO_EMISSION, Params *params, int verbose)
{
  ...
  double Ecov[NDIM][NDIM], Econ[NDIM][NDIM];
  double complex N_tetrad[NDIM][NDIM], N_preserve[NDIM][NDIM];
  double B;
  double jI, jQ, jU, jV;
  double aI, aQ, aU, aV;
  double rV, rU, rQ;
  ...
  double SI0_2, SQ0_2, SU0_2, SV0_2;
  double SI0_c, SQ0_c, SU0_c, SV0_c;
  double SI0_c2, SQ0_c2, SU0_c2, SV0_c2;
  double SI1, SQ1, SU1, SV1;
  double SI2, SQ2, SU2, SV2;
  double SI_c, SQ_c, SU_c, SV_c;
  int oddflag = 0;
  ...
  if (SI0_c2-SI0_c > 1e-50) {
    ...
    complex_coord_to_tetrad_rank2(N_coord, Ecov, N_tetrad);
    complex_tetrad_to_coord_rank2(N_tetrad, Econ, N_coord);
    tensor_to_stokes(N_coord, &SI0_c2, &SQ0_c2, &SU0_c2, &SV0_c2);
    fprintf(stderr, "Coord-only post: %g %g %g %g\n", SI0_c2, SQ0_c2, SU0_c2, SV0_c2);
    fprintf(stderr, "Diffs: %g %g %g %g\n", SI0_c2-SI0_c, SQ0_c2-SQ0_c, SU0_c2-SU0_c, SV0_c2-SV0_c);

    tensor_to_stokes(N_tetrad, &SI0, &SQ0, &SU0, &SV0);
    print_matrix_c("Tetrad-only pre", N_tetrad);
    stokes_to_tensor(SI0, SQ0, SU0, SV0, N_tetrad);
    tensor_to_stokes(N_tetrad, &SI0_2, &SQ0_2, &SU0_2, &SV0_2);
    print_matrix_c("Tetrad-only post", N_tetrad);
    //fprintf(stderr, "Diffs: %g %g %g %g\n", SI0_2-SI0, SQ0_2-SQ0, SU0_2-SU0, SV0_2-SV0);

    complex_tetrad_to_coord_rank2(N_tetrad, Econ, N_coord);
    tensor_to_stokes(N_coord, &SI0_c2, &SQ0_c2, &SU0_c2, &SV0_c2);
    fprintf(stderr, "Both post: %g %g %g %g\n", SI0_c2, SQ0_c2, SU0_c2, SV0_c2);
    fprintf(stderr, "Diffs: %g %g %g %g\n", SI0_c2-SI0_c, SQ0_c2-SQ0_c, SU0_c2-SU0_c, SV0_c2-SV0_c);
  } else {
    complex_coord_to_tetrad_rank2(N_coord, Ecov, N_tetrad);
    tensor_to_stokes(N_tetrad, &SI0, &SQ0, &SU0, &SV0);
  ...
  if (aP > 100) {
    // Solution assuming aI ~ aP > 1000, the worst case.
    ...
    // Full original analytic solution, for most cases
    ...
    SQ2 = (SQ1 * expaIx
        ...
        + jQ * (1 - expaIx) / aI
        + adj * aQ / (aI * (aI2 - aP2)) *
        (1 - (1 - aI2 / aP2) * expaIx - aI / aP2 * (aI * coshaPx + aP * sinhaPx) * expaIx)
  ...
  *tauF += dlam*fabs(rV); //*sqrt(SQ*SQ + SU*SU);
  ...
  stokes_to_tensor(SI, SQ, SU, SV, N_tetrad);
  complex_tetrad_to_coord_rank2(N_tetrad, Econ, N_coord);
  tensor_to_stokes(N_coord, &SI_c, &SQ_c, &SU_c, &SV_c);
  ...
  // Cry on bad N
  if (oddflag & 4) {
    fprintf(stderr, "\nNaN in N00 (solution step)!\n");
    fprintf(stderr, "dlam: %g\n", dlam);
    fprintf(stderr, "j: %g %g %g %g\n", jI, jQ, jU, jV);
    fprintf(stderr, "alpha: %g %g %g %g\n", aI, aQ, aU, aV);
    fprintf(stderr, "rho: %g %g %g\n", rQ, rU, rV);
    fprintf(stderr, "Stokes pre-solve: %g %g %g %g\n", SI0, SQ0, SU0, SV0);
    fprintf(stderr, "Stokes rot1: %g %g %g %g\n", SI1, SQ1, SU1, SV1);
    fprintf(stderr, "Stokes em-abs: %g %g %g %g\n", SI2, SQ2, SU2, SV2);
    fprintf(stderr, "Stokes post-solve: %g %g %g %g\n", SI, SQ, SU, SV);
    fprintf(stderr, "aI2: %g aP2: %g aP: %g ads0: %g adj: %g expaIx: %g sinhaPx: %g coshaPx: %g\n",
            aI2, aP2, aP, ads0, adj, expaIx, sinhaPx, coshaPx);
    print_vector("Xi", Xi);
    print_vector("Xm", Xhalf);
    print_vector("Xf", Xf);
    print_vector("Ki", Kconi);
    print_vector("Km", Kconhalf);
    print_vector("Kf", Kconf);
    print_matrix_c("N_t", N_tetrad);
    print_matrix_c("N_c", N_coord);
    //exit(-1);
  }
