...
REAL get_dump_t(char *fnam, int dumpidx);
...
REAL get_dump_t(char *fnam, int dumpidx)
{
  ...
  hdf5_open(fname);
  hdf5_set_directory("/");
  hdf5_read_single_val(&td, "/t", H5T_DOUBLE);
  hdf5_close();
                     ...
                     REAL Bcon[NDIM], REAL Bcov[NDIM])
{
  ...
  double Vcon[NDIM];
  Vcon[1] = interp_scalar(X, data[0]->p[U1]);
  Vcon[2] = interp_scalar(X, data[0]->p[U2]);
  Vcon[3] = interp_scalar(X, data[0]->p[U3]);
  ...
  // interpolate primitive variables first
  double Bcon1, Bcon2, Bcon3;
  Bcon1 = interp_scalar(X, data[0]->p[B1]);
  Bcon2 = interp_scalar(X, data[0]->p[B2]);
  Bcon3 = interp_scalar(X, data[0]->p[B3]);
...
void get_model_primitives(REAL X[NDIM], REAL *p)
{
  ...
  for (int np=0; np<8; np++) {
    p[np] = interp_scalar(X, data[0]->p[np]);
  }
}
...
double get_model_thetae(REAL X[NDIM])
{
  ...
  double thetae = interp_scalar(X, data[0]->thetae);

  if (thetae < 0.) {
    printf("thetae negative!\n");
    printf("X[] = %Lg %Lg %Lg %Lg\n", X[0], X[1], X[2], X[3]);
...
double get_model_b(REAL X[NDIM])
{
  ...
  double b = interp_scalar(X, data[0]->b);
...
double get_model_ne(REAL X[NDIM])
{
  ...
  double ne = interp_scalar(X, data[0]->ne);
...
void init_storage(void)
{
  ...
  for (int n = 0; n < NSUP; n++) {
    data[n]->bcon = malloc_rank4_double(N1+2,N2+2,N3+2,NDIM);
    data[n]->bcov = malloc_rank4_double(N1+2,N2+2,N3+2,NDIM);
    data[n]->ucon = malloc_rank4_double(N1+2,N2+2,N3+2,NDIM);
    data[n]->ucov = malloc_rank4_double(N1+2,N2+2,N3+2,NDIM);
    data[n]->p = malloc_rank4_double(NVAR,N1+2,N2+2,N3+2);
    data[n]->ne = malloc_rank3_double(N1+2,N2+2,N3+2);
    data[n]->thetae = malloc_rank3_double(N1+2,N2+2,N3+2);
    data[n]->b = malloc_rank3_double(N1+2,N2+2,N3+2);
  }
}
...
//  hdf5_read_single_val(&t0, "t", H5T_DOUBLE);
  ...
  hdf5_read_single_val(&gam, "gam", H5T_DOUBLE);

  if (hdf5_exists("gam_e")) {
    fprintf(stderr, "custom electron model loaded from dump file...\n");
    hdf5_read_single_val(&game, "gam_e", H5T_DOUBLE);
    hdf5_read_single_val(&gamp, "gam_p", H5T_DOUBLE);
  ...
  if (!USE_FIXED_TPTE && !USE_MIXED_TPTE) {
    ...
    fprintf(stderr, "Using fixed tp_over_te ratio = %Lg\n", tp_over_te);
    ...
    ELECTRONS = 2;
    fprintf(stderr, "Using mixed tp_over_te with trat_small = %Lg, trat_large = %Lg, and beta_crit = %Lg\n", 
  ...
  if (RADIATION) {
    ...
    hdf5_read_single_val(&M_unit, "M_unit", H5T_DOUBLE);
    hdf5_read_single_val(&T_unit, "T_unit", H5T_DOUBLE);
    hdf5_read_single_val(&L_unit, "L_unit", H5T_DOUBLE);
    hdf5_read_single_val(&Thetae_unit, "Thetae_unit", H5T_DOUBLE);
    hdf5_read_single_val(&MBH, "Mbh", H5T_DOUBLE);
    hdf5_read_single_val(&tp_over_te, "tp_over_te", H5T_DOUBLE);
  }

  hdf5_set_directory("/header/geom/");
  hdf5_read_single_val(&startx[1], "startx1", H5T_DOUBLE);
  hdf5_read_single_val(&startx[2], "startx2", H5T_DOUBLE);
  hdf5_read_single_val(&startx[3], "startx3", H5T_DOUBLE);
  hdf5_read_single_val(&dx[1], "dx1", H5T_DOUBLE);
  hdf5_read_single_val(&dx[2], "dx2", H5T_DOUBLE);
  hdf5_read_single_val(&dx[3], "dx3", H5T_DOUBLE);
  ...
  if ( metric == METRIC_MKS3 ) {
    hdf5_read_single_val(&a, "a", H5T_DOUBLE);
    hdf5_read_single_val(&mks3R0, "R0", H5T_DOUBLE);
    hdf5_read_single_val(&mks3H0, "H0", H5T_DOUBLE);
    hdf5_read_single_val(&mks3MY1, "MY1", H5T_DOUBLE);
    hdf5_read_single_val(&mks3MY2, "MY2", H5T_DOUBLE);
    hdf5_read_single_val(&mks3MP0, "MP0", H5T_DOUBLE);
    Rout = 100.; 
  } else { // Some brand of MKS.  All have the same parameters
    hdf5_read_single_val(&a, "a", H5T_DOUBLE);
    hdf5_read_single_val(&hslope, "hslope", H5T_DOUBLE);
    if (hdf5_exists("Rin")) {
      hdf5_read_single_val(&Rin, "Rin", H5T_DOUBLE);
      hdf5_read_single_val(&Rout, "Rout", H5T_DOUBLE);
    } else {
      hdf5_read_single_val(&Rin, "r_in", H5T_DOUBLE);
      hdf5_read_single_val(&Rout, "r_out", H5T_DOUBLE);
    }
    fprintf(stderr, "MKS parameters a: %Lf hslope: %Lf Rin: %Lf Rout: %Lf\n", a, hslope, Rin, Rout);

    if (metric == METRIC_FMKS) {
      hdf5_read_single_val(&poly_xt, "poly_xt", H5T_DOUBLE);
      hdf5_read_single_val(&poly_alpha, "poly_alpha", H5T_DOUBLE);
      hdf5_read_single_val(&mks_smooth, "mks_smooth", H5T_DOUBLE);
      poly_norm = 0.5*M_PI*1./(1. + 1./(poly_alpha + 1.)*1./pow(poly_xt, poly_alpha));
      fprintf(stderr, "MKS parameters poly_xt: %Lf poly_alpha: %Lf mks_smooth: %Lf poly_norm: %Lf\n", poly_xt, poly_alpha, mks_smooth, poly_norm);
    }
  }
  ...
  hdf5_set_directory("/");
  hdf5_read_single_val(&DTd, "dump_cadence", H5T_DOUBLE);
  ...
  fprintf(stderr, "Native coordinate start: %Lg %Lg %Lg stop: %Lg %Lg %Lg\n",
...
void output_hdf5()
{
  ...
  hdf5_write_single_val(&Mdot_dump, "Mdot", H5T_DOUBLE);
  hdf5_write_single_val(&MdotEdd_dump, "MdotEdd", H5T_DOUBLE);
  hdf5_write_single_val(&Ladv_dump, "Ladv", H5T_DOUBLE);

  hdf5_set_directory("/header/");
  hdf5_write_single_val(&(data[0]->t), "t", H5T_DOUBLE);

  hdf5_write_single_val(&sigma_cut, "sigma_cut", H5T_DOUBLE);
  hdf5_make_directory("electrons");
  hdf5_set_directory("/header/electrons/");
  if (ELECTRONS == 0) {
    hdf5_write_single_val(&tp_over_te, "tp_over_te", H5T_DOUBLE);
  } else if (ELECTRONS == 2) {
    hdf5_write_single_val(&trat_small, "rlow", H5T_DOUBLE);
    hdf5_write_single_val(&trat_large, "rhigh", H5T_DOUBLE);
    hdf5_write_single_val(&beta_crit, "beta_crit", H5T_DOUBLE);
  }
...
void load_iharm_data(int n, char *fnam, int dumpidx, int verbose)
{
  ...
  fstart[3] = 0;
  hdf5_read_array(data[n]->p[KRHO][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 1;
  hdf5_read_array(data[n]->p[UU][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 2;
  hdf5_read_array(data[n]->p[U1][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 3;
  hdf5_read_array(data[n]->p[U2][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 4;
  hdf5_read_array(data[n]->p[U3][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 5;
  hdf5_read_array(data[n]->p[B1][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 6;
  hdf5_read_array(data[n]->p[B2][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  fstart[3] = 7;
  hdf5_read_array(data[n]->p[B3][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE); 

  if (ELECTRONS == 1) {
    fstart[3] = 8;
    hdf5_read_array(data[n]->p[KEL][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
    fstart[3] = 9;
    hdf5_read_array(data[n]->p[KTOT][0][0], "prims", 4, fdims, fstart, fcount, mdims, mstart, H5T_DOUBLE);
  }

  hdf5_read_single_val(&(data[n]->t), "t", H5T_DOUBLE);

  hdf5_close();

  dMact = Ladv = 0.;

  // construct four-vectors over "double" zones
...
#pragma omp parallel for collapse(2)
  ...
  if (verbose == 2) {
    fprintf(stderr,"dMact: %Lg [code]\n",dMact);
    fprintf(stderr,"Ladv: %Lg [code]\n",Ladv_dump);
    fprintf(stderr,"Mdot: %Lg [g/s] \n",Mdot_dump);
    fprintf(stderr,"Mdot: %Lg [MSUN/YR] \n",Mdot_dump/(MSUN / YEAR));
    fprintf(stderr,"Mdot: %Lg [Mdotedd]\n",Mdot_dump/MdotEdd_dump);
    fprintf(stderr,"Mdotedd: %Lg [g/s]\n",MdotEdd_dump);
    fprintf(stderr,"Mdotedd: %Lg [MSUN/YR]\n",MdotEdd_dump/(MSUN/YEAR));
  } else if (verbose == 1) {
    fprintf(stderr,"Mdot: %Lg [MSUN/YR] \n",Mdot_dump/(MSUN / YEAR));
    fprintf(stderr,"Mdot: %Lg [Mdotedd]\n",Mdot_dump/MdotEdd_dump);
  }
